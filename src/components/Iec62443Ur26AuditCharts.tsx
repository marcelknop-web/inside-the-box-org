...
export function Iec62443AuditCharts({ threats, reqs }: { threats: IecThreat[]; reqs: IecReq[] }) {
  ...
  const topRisks = useMemo(() => {
    return [...threats]
      ...
      .slice(0, 6)
      .map(th => ({
        name: `${th.fr}-${String(th.id).padStart(3, '0')}`,
    ...
    <div className="space-y-4">
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        <div className="bg-card border border-border rounded-lg p-4">
          <h4 className="text-sm font-bold text-foreground mb-3">E26 Chapters</h4>
          <ResponsiveContainer width="100%" height={220}>
            <RadarChart data={frData}>
              <PolarGrid stroke="hsl(var(--border))" />
              <PolarAngleAxis dataKey="category" tick={{ fill: 'hsl(var(--foreground))', fontSize: 9, fontWeight: 700 }} />
        ...
        {/* Compliance Donut */}
        <div className="bg-card border border-border rounded-lg p-4">
          <h4 className="text-sm font-bold text-foreground mb-3">IACS UR E26 Compliance</h4>
          <ResponsiveContainer width="100%" height={220}>
            <PieChart>
