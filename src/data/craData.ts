...
export const THREATS: Threat[] = [
  ...
  { id: 8, stride: 'E', name: 'Session-Hijacking Web-UI', component: 'Web-UI Admin', attacker: 'Netzwerk-Angreifer', path: 'Unsicheres Session-Management → Token-Diebstahl → Admin-Zugriff ohne Authentifizierung', cra: 'Annex I, Part I, Nr. 3', likelihood: 3, impact: 4,
    evidence: 'Cookie-Analyse (Browser DevTools → Application → Cookies): Session-Token "sid" ohne Secure-Flag (Secure=false), ohne HttpOnly-Flag. curl -v -b "sid=<stolen-token>" https://172.16.0.20/api/config → HTTP 200 mit Admin-Konfigurationsdaten. Token-Rotation nach Login findet nicht statt (gleicher Token-Wert nach 3 aufeinanderfolgenden Logins). Session-Timeout: 24h (konfiguriert), kein Re-Auth bei sensitiven Aktionen (Passwort-Änderung, Firmware-Upload).',
    rationale: 'Likelihood 3: Erfordert MITM-Position oder XSS-Vektor im selben Netzwerk, nicht trivial, aber in OT-Umgebungen mit unverschlüsseltem HTTP realistisch. Impact 4: Vollständiger Admin-Zugriff mit gestohlener Session, aber zeitlich begrenzt auf Session-Lebensdauer (24h).',
...
export const CRA_REQS: CraReq[] = [
  ...
  { id: 'A1-2', article: 'Annex I, Part I, Nr. 2', name: 'Secure by Default', status: 'fail', gap: 'Standard-Passwort aktiv, Debug-Endpoint im Produktionsmodus, SSH Root-Login erlaubt',
    ...
    measure: '1. Passwort-Änderung beim Erststart erzwingen (min. 12 Zeichen, 3 von 4 Komplexitätsklassen, Blacklist Top-10.000). 2. Debug-Endpoints in Produktion deaktivieren (Build-Flag). 3. SSH: PermitRootLogin=no, PasswordAuthentication=no, Key-Only. 4. Port 1883 im Default deaktivieren, nur 8883 (TLS).',
    ...
    effort: '16–24h (Backend 8–12h, Frontend 4–6h, QA 3–4h, Security-Review 1–2h)', priority: 'P0' },
  ...
  { id: 'A1-3', article: 'Annex I, Part I, Nr. 3', name: 'Schutz vor unbefugtem Zugriff', status: 'fail', gap: 'Kein MFA, schwaches Session-Management, abgelaufene Tokens nicht revoziert',
    ...
    measure: '1. MFA für Admin implementieren (TOTP via Google Authenticator/Authy oder FIDO2 WebAuthn). 2. Session-Cookies: Secure=true, HttpOnly=true, SameSite=Strict; Token-Rotation nach Login; Timeout ≤ 30 min. 3. Token-Revocation bei Offboarding automatisieren (HR-Webhook → API). 4. RBAC auf Endpoint-Ebene mit Berechtigungsmatrix.',
    ...
    effort: '40–56h (MFA 20–30h, Session-Fix 8–12h, Token-Revocation 4–6h, RBAC 8–10h)', priority: 'P0' },
  ...
  { id: 'A1-4', article: 'Annex I, Part I, Nr. 4', name: 'Vertraulichkeit der Daten', status: 'partial', gap: 'MQTT auf Port 1883 unverschlüsselt, OPC-UA Security Mode None',
    ...
    measure: '1. MQTT ausschließlich über TLS 1.2+ (Port 8883), Port 1883 in Firewall-Regeln und Konfiguration deaktivieren. 2. OPC-UA: SecurityMode=SignAndEncrypt, SecurityPolicy=Basic256Sha256. 3. Mutual TLS (mTLS) für Client-Authentifizierung aktivieren.',
    ...
    effort: '20–32h (MQTT-TLS 12–16h, OPC-UA-Config 4–8h, mTLS 4–8h)', priority: 'P0' },
  ...
  { id: 'A1-7', article: 'Annex I, Part I, Nr. 7', name: 'Verfügbarkeit & Ausfallsicherheit', status: 'partial', gap: 'Kein Rate-Limiting auf MQTT-Broker und REST-API',
    ...
    measure: '1. Rate-Limiting pro Client-IP auf MQTT (max. 50 Connections/min) und REST (max. 100 Requests/min). 2. Connection-Throttling mit konfigurierbarem Maximum (z. B. max_clients=200). 3. Watchdog Recovery-Zeit auf < 10s optimieren (systemd Restart=always, RestartSec=3).',
  ...
  { id: 'A1-8', article: 'Annex I, Part I, Nr. 8', name: 'Sicherheits-Logging & Monitoring', status: 'fail', gap: 'Admin-Aktionen nicht protokolliert, kein SIEM-Export',
    ...
    measure: '1. Audit-Log für alle Admin-Aktionen implementieren (JSON-Format: Zeitstempel ISO-8601, User-ID, Aktion, Vorher/Nachher-Wert, Source-IP). 2. Log-Rotation (logrotate, max. 90 Tage) und Tamper-Protection (Append-Only via chattr +a). 3. Syslog/SIEM-Export konfigurierbar (rsyslog → TCP 514, TLS-gesichert).',
    ...
    effort: '16–24h (Backend-Logging 8–12h, Log-Rotation 2–4h, SIEM-Export 4–8h)', priority: 'P1' },
  ...
  { id: 'A2-1', article: 'Annex I, Part II, Nr. 1', name: 'Schwachstellen-Identifikation', status: 'partial', gap: 'Ad-hoc-Prozess vorhanden, aber nicht dokumentiert oder regelmäßig',
    ...
    measure: '1. Vulnerability-Management-Prozess gemäß ISO/IEC 30111 dokumentieren (Rollen, Verantwortlichkeiten, Eskalation). 2. SAST (z. B. SonarQube, Semgrep) und DAST (z. B. ZAP, Nuclei) in CI/CD integrieren. 3. Jährliche Pentests durch externe Prüfer (z. B. BSI-zertifizierte Prüfstelle).',
    ...
    effort: '20–32h (Prozess-Doku 8h, SAST/DAST-Integration 8–16h, Pentest-Beauftragung 4–8h)', priority: 'P2' },
  ...
  { id: 'A2-6', article: 'Annex I, Part II, Nr. 6', name: 'Retroaktive Patches (ältere Versionen)', status: 'partial', gap: 'Nur aktuelle Major-Version wird gepatcht',
    ...
    measure: '1. End-of-Support-Policy dokumentieren und auf Webseite publizieren. 2. Sicherheitskritische Patches für v2.x mindestens 12 Monate bereitstellen. 3. Upgrade-Pfad von v2.x auf v3.x vereinfachen (Migrations-Tool).',
    ...
    effort: '16–24h (Policy 4h, Backport-Patches 8–16h, Migrations-Tool 4h)', priority: 'P2' },
  ...
  { id: 'A2-8', article: 'Annex I, Part II, Nr. 8', name: 'SBOM', status: 'fail', gap: 'Keine vollständige SBOM vorhanden',
    ...
    measure: '1. SBOM-Generierung in CI/CD integrieren (Syft für Container, Trivy für OS-Packages). 2. Output-Format: CycloneDX 1.5 (JSON). 3. Transitive Dependencies einschließen. 4. Bei jedem Release automatisch generieren und als Build-Artefakt publizieren.',
    ...
    effort: '8–16h (Tool-Evaluation 2h, CI/CD-Integration 4–8h, Validierung 2–4h, Doku 2h)', priority: 'P2' },
  ...
  { id: 'Art11', article: 'Artikel 11', name: 'Cybersecurity Testing & Zertifizierung', status: 'partial', gap: 'Testing-Framework nicht formalisiert, keine externe Zertifizierung',
    ...
    measure: '1. Security-Testing-Framework formalisieren (OWASP ASVS Level 2 als Basis). 2. Testabdeckung für Security-Funktionen auf > 85% erhöhen. 3. Externe Konformitätsbewertung nach Art. 24 CRA beauftragen.',
    ...
    effort: '24–40h (Framework-Doku 8h, Test-Coverage 12–24h, Externe Beauftragung 4–8h)', priority: 'P3' },
  ...
  { id: 'Art13', article: 'Artikel 13', name: 'Technische Dokumentation', status: 'partial', gap: 'Unvollständige Architektur-Doku, keine formale Risikoanalyse',
    ...
    measure: '1. Architektur-Dokumentation vervollständigen (alle Schnittstellen, Datenflüsse, Sicherheitszonen). 2. Formale Risikoanalyse gemäß Annex VII erstellen (basierend auf diesem STRIDE-Assessment). 3. EU-Konformitätserklärung nach Art. 28 vorbereiten.',
    ...
    effort: '16–28h (Architektur-Doku 8–12h, Risikoanalyse 4–8h, Konformitätserklärung 4–8h)', priority: 'P2' },
  ...
  { id: 'Art14', article: 'Artikel 14', name: 'Meldepflichten (24h/72h)', status: 'fail', gap: 'Kein Incident-Response-Prozess, keine ENISA-Melderoute',
    ...
    measure: '1. IR-Prozess dokumentieren (Rollen: IR-Lead, Kommunikation, Technik; Eskalationsstufen: Sev1/2/3; Zeitvorgaben: 24h/72h). 2. ENISA-Meldeplattform registrieren und Zugang testen. 3. Trockenübung (Tabletop Exercise) mit realem Szenario durchführen. 4. On-Call-Rotation für Security-Incidents einrichten (PagerDuty/Opsgenie).',
