...
export const OT_REASON_OVERRIDES: Record<string, Reason> = {
  ...
  "plc_write_anomaly::verify::active_scan":   R("Active scans on a live OT segment can fault legacy PLCs mid-batch — never your first move."),
  "plc_write_anomaly::verify::ping_plc":      R("Even a probe can knock fragile PLCs offline. Touching the process before you understand it is the OT cardinal sin."),
  ...
  "ews_compromise::scope::ask_engineer":      R("Calling the engineer over an unverified channel can tip off the attacker if their account is compromised."),
  ...
  "sis_bypass::contain::stop_process":        R("Shutting down a burner mid-state can be more dangerous than the threat. SIS is your last line — keep it intact, not the process."),
  ...
  "vendor_remote_access::verify::kill_now":        R("Hard-killing a legitimate vendor session mid-write can corrupt PLC programs. Verify before you sever."),
  ...
  "ics_ransomware_dmz::isolate::leave_running":R("Leaving an actively-encrypting host running for 'more IOCs' is how IDMZ incidents become plant incidents."),
  ...
  "historian_tampering::preserve::live_query":  R("Live queries on production change file timestamps and pollute the very evidence you need."),
  ...
  "asset_scan_storm::verify::restart_plc":      R("Restarting a PLC in fault without understanding the cause can compound the upset and confuse engineering."),
  ...
  "l2_worm_outbreak::recover::reflash_plc":     R("Re-flashing PLCs without evidence of compromise risks process upset for no measurable benefit."),
  ...
  "hmi_default_creds::verify::live_login":      R("Logging in 'just to confirm' creates auditor questions about your own conduct and may trigger detections."),
  ...
  "ups_tampering::react::go_alone":             R("Walking into a substation alone during a possible intrusion is a personal-safety violation, not just procedure."),
