...
export const IEC_REQS: IecReq[] = [
  ...
  { id: 'UC-1', article: 'E27-08 (SR 2.1)', name: 'Authorization Enforcement', status: 'fail',
    gap: 'Keine rollenbasierte Zugriffssteuerung auf dem Bridge-System; einheitliche Berechtigungsstufe für alle Benutzer',
    evidence: 'Das Bridge-System verfügt über keine Rollendifferenzierung. Jeder angemeldete Benutzer besitzt volle Konfigurationsrechte.',
    rationale: 'Die Anforderung ist nicht erfüllt. Die fehlende rollenbasierte Zugriffssteuerung verletzt das Least-Privilege-Prinzip und ermöglicht unbeabsichtigte oder böswillige Konfigurationsänderungen.',
    measure: '1. Konfiguration einer rollenbasierten Zugriffssteuerung (Kapitän, Offizier, Service). 2. Dokumentation einer Berechtigungsmatrix.',
    criteria: ['Mindestens drei Rollen mit differenzierten Berechtigungen konfiguriert', 'Berechtigungsmatrix dokumentiert und freigegeben'],
    effort: '24-32h', priority: 'P1' },
  { id: 'UC-2', article: 'E27-10 (SR 2.3)', name: 'Portable & Mobile Devices', status: 'partial',
    gap: 'USB-Ports nicht deaktiviert; kein Wechselmedien-Konzept vorhanden',
    evidence: 'Die USB-Ports an ECDIS- und Engine-Workstations sind physisch zugänglich und nicht gesperrt. Ein formales Wechselmedien-Konzept liegt nicht vor.',
    rationale: 'Die Anforderung ist teilweise erfüllt. Eine organisatorische Regelung zur USB-Nutzung besteht, wird jedoch technisch nicht durchgesetzt.',
    measure: '1. Technische Deaktivierung der USB-Ports per BIOS/Software. 2. Einrichtung einer dedizierten Kiosk-Station für Kartenupdates. 3. Erstellung einer Wechselmedien-Policy.',
    criteria: ['USB-Ports an allen CBS technisch deaktiviert (Ausnahme: dedizierte Update-Stationen)', 'Wechselmedien-Policy dokumentiert und kommuniziert'],
    effort: '12-20h', priority: 'P1' },
  { id: 'UC-3', article: 'E27-12 (SR 2.5)', name: 'Session Lock', status: 'partial',
    gap: 'Keine automatische Session-Sperre auf Brücken-CBS konfiguriert',
    evidence: 'ECDIS- und Radar-Workstations verfügen über keine automatische Bildschirmsperre. Sessions bleiben permanent aktiv.',
    rationale: 'Die Anforderung ist teilweise erfüllt. Eine manuelle Sperre ist möglich, ein automatischer Timeout ist jedoch nicht konfiguriert.',
    measure: 'Konfiguration einer automatischen Session-Sperre nach 15 Minuten Inaktivität auf allen CBS.',
    criteria: ['Automatische Session-Sperre nach maximal 15 Minuten auf allen CBS aktiv'],
  ...
  { id: 'SI-1', article: 'E27-18 (SR 3.2)', name: 'Malicious Code Protection', status: 'fail',
    ...
    evidence: 'Die ECDIS-Workstation basiert auf Windows, verfügt jedoch weder über ein Antivirus-Programm noch über Application Whitelisting. Das letzte Betriebssystem-Update liegt 18 Monate zurück.',
    rationale: 'Die Anforderung ist nicht erfüllt. Das Fehlen jeglichen Malware-Schutzes auf Safety-kritischen CBS stellt ein erhebliches Risiko dar.',
    measure: '1. Implementierung von Application Whitelisting auf allen CBS. 2. Einrichtung von Offline-Signatur-Updates. 3. Etablierung regelmäßiger Scans.',
    criteria: ['Application Whitelisting auf allen CBS aktiv', 'Signatur-Updates maximal 30 Tage alt'],
    effort: '16-24h', priority: 'P0' },
  { id: 'SI-2', article: 'E27-17 (SR 3.1)', name: 'Communication Integrity', status: 'fail',
    gap: 'NMEA-Kommunikation ohne Integritätsschutz',
    evidence: 'Der NMEA-Datenstrom wird unverschlüsselt und ohne Integritätsprüfung übertragen. Ein GPS-Spoofing-PoC war erfolgreich.',
    rationale: 'Die Anforderung ist nicht erfüllt. Manipulierte Navigationsdaten werden vom System ohne jegliche Prüfung akzeptiert.',
    measure: '1. Installation eines NMEA-Gateways mit Integritätsprüfung. 2. Implementierung eines Multi-Source-Vergleichs (GPS/GLONASS/Galileo). 3. Einrichtung einer AIS-Kreuzprüfung.',
  ...
  { id: 'SI-3', article: 'E27-20 (SR 3.5)', name: 'Input Validation', status: 'partial',
    gap: 'Keine Validierung externer Eingaben über Untrusted Networks',
    evidence: 'Daten des Fleet-Management-Systems werden ohne Validierung verarbeitet. Eine Input-Sanitization findet nicht statt.',
    rationale: 'Die Anforderung ist teilweise erfüllt. Die interne Datenverarbeitung erfolgt validiert, externe Eingaben werden jedoch nicht geprüft.',
    measure: 'Implementierung einer Input-Validierung für alle Daten, die über Untrusted Networks empfangen werden.',
    criteria: ['Input-Validierung für alle externen Datenquellen implementiert und dokumentiert'],
    effort: '16-24h', priority: 'P2' },
  { id: 'SI-4', article: 'E27-21 (SR 3.6)', name: 'Deterministic Output', status: 'pass',
    gap: '',
    evidence: 'Das Engine Control System geht bei CBS-Ausfall in einen definierten Safe-State über (Dead Ship Condition Procedure). Das ECDIS verfügt über eine papierbasierte Rückfallebene.',
    rationale: 'Die Anforderung ist erfüllt. Das Fail-Safe-Verhalten ist dokumentiert und wurde getestet.',
  ...
  { id: 'DC-1', article: 'E27-22 (SR 4.1)', name: 'Information Confidentiality', status: 'fail',
    ...
    evidence: 'Es wurden HTTP-Verbindungen zum Fleet-Management-System ohne TLS-Verschlüsselung festgestellt. Crew-Login-Daten werden im Klartext übertragen.',
    rationale: 'Die Anforderung ist nicht erfüllt. Betriebsdaten und personenbezogene Daten werden unverschlüsselt über die VSAT-Verbindung übertragen.',
    measure: '1. Einführung von TLS 1.2+ für alle Shore-Verbindungen. 2. VPN-Tunnel für das Fleet-Management. 3. Verschlüsselung at Rest auf CBS.',
  ...
  { id: 'DC-2', article: 'E27-23 (SR 4.3)', name: 'Use of Cryptography', status: 'partial',
    gap: 'Veraltete Kryptographie-Algorithmen; kein Zertifikatsmanagement vorhanden',
    evidence: 'Auf einigen Systemen ist noch TLS 1.0 aktiv. Die eingesetzten Zertifikate sind selbstsigniert. Ein Zertifikats-Lifecycle-Management existiert nicht.',
    rationale: 'Die Anforderung ist teilweise erfüllt. Kryptographie wird eingesetzt, ist jedoch veraltet und wird nicht zentral verwaltet.',
    measure: 'Durchsetzung von TLS 1.2+ auf allen Systemen. Aufbau eines zentralen Zertifikatsmanagements.',
    criteria: ['TLS 1.0/1.1 deaktiviert', 'Zertifikats-Lifecycle-Management etabliert'],
    effort: '16-24h', priority: 'P2' },

  // AL — Audit & Logging
  { id: 'AL-1', article: 'E27-13 (SR 2.8)', name: 'Auditable Events', status: 'fail',
    gap: 'Keine ausreichende Event-Protokollierung auf CBS',
    evidence: 'Es ist weder ein Intrusion-Detection-System noch eine zentrale Log-Aggregation vorhanden. Die Logs der Einzelsysteme werden nicht systematisch ausgewertet.',
    rationale: 'Die Anforderung ist nicht erfüllt. Security-relevante Ereignisse werden nicht systematisch erfasst und können daher nicht nachvollzogen werden.',
    measure: '1. Aufbau einer zentralen Log-Aggregation (Syslog-Server). 2. Installation eines IDS im Bordnetzwerk. 3. Erstellung einer Event-Monitoring-Policy.',
    criteria: ['Zentrale Log-Aggregation für alle CBS vorhanden', 'IDS in kritischen Netzwerksegmenten installiert'],
    effort: '40-60h', priority: 'P0' },
  { id: 'AL-2', article: 'E27-16 (SR 2.11)', name: 'Timestamps', status: 'fail',
    gap: 'Keine NTP-Synchronisation; Uhrzeiten der CBS weichen erheblich ab',
    evidence: 'Die Uhrzeiten der CBS-Systeme weichen um bis zu 47 Minuten voneinander ab. Ein NTP-Server ist an Bord nicht installiert.',
    rationale: 'Die Anforderung ist nicht erfüllt. Ohne Zeitsynchronisation ist eine Log-Korrelation und damit eine wirksame Forensik nicht möglich.',
    measure: 'Installation eines NTP-Servers an Bord und Synchronisation aller CBS.',
    criteria: ['NTP-Server an Bord aktiv', 'Alle CBS-Uhrzeiten synchronisiert (max. ±1 s Abweichung)'],
  ...
  { id: 'RA-1', article: 'E27-25 (SR 7.1)', name: 'Denial of Service Protection', status: 'fail',
    ...
    evidence: 'Das ECDIS wird auf einzelner Hardware ohne Backup betrieben. Ein Backup-ECDIS ist zwar physisch vorhanden, jedoch nicht konfiguriert.',
    rationale: 'Die Anforderung ist nicht erfüllt. Ein Single Point of Failure besteht bei einem SOLAS-relevanten System.',
    measure: '1. Konfiguration des ECDIS-Hot-Standby. 2. Einrichtung eines automatischen Failovers. 3. Durchführung regelmäßiger Failover-Tests.',
    criteria: ['ECDIS-Failover konfiguriert und getestet', 'Halbjährlicher Failover-Test dokumentiert'],
    effort: '24-40h', priority: 'P0' },
  { id: 'RA-2', article: 'E27-27 (SR 7.3)', name: 'System Backup', status: 'fail',
    gap: 'Keine dokumentierte Backup-Strategie für CBS',
    evidence: 'Ein Recovery-Plan liegt nicht vor. Backup-Prozeduren sind nicht dokumentiert. Der Zeitpunkt des letzten Backups ist nicht bekannt.',
    rationale: 'Die Anforderung ist nicht erfüllt. Für Safety-relevante CBS existiert keine Backup-Strategie.',
    measure: '1. Erstellung einer Backup-Policy für alle CBS. 2. Einrichtung regelmäßiger automatisierter Backups. 3. Halbjährliche Restore-Tests.',
    criteria: ['CBS-Backup-Policy dokumentiert', 'Automatisierte Backups aktiv', 'Halbjährliche Restore-Tests durchgeführt'],
    effort: '16-24h', priority: 'P1' },
  { id: 'RA-3', article: 'E27-28 (SR 7.4)', name: 'System Recovery & Reconstitution', status: 'partial',
    gap: 'Kein CBS-spezifischer Recovery-Plan für Szenarien auf See',
    evidence: 'Ein generischer IT-Recovery-Plan ist vorhanden. CBS-spezifische Recovery-Prozeduren für Szenarien auf See fehlen jedoch.',
    rationale: 'Die Anforderung ist teilweise erfüllt. Ein generischer Plan existiert, eine Erweiterung um maritim-spezifische Szenarien steht noch aus.',
    measure: 'Erstellung eines CBS-spezifischen Recovery-Plans mit maritimen Szenarien (Ausfall auf See, eingeschränkte Kommunikation).',
    criteria: ['CBS-Recovery-Plan mit See-Szenarien dokumentiert', 'Jährlicher Recovery-Test durchgeführt'],
    effort: '16-24h', priority: 'P1' },
  { id: 'RA-4', article: 'E27-29 (SR 7.5)', name: 'Emergency Power', status: 'pass',
    gap: '',
    evidence: 'Eine Notstromversorgung ist für alle Essential CBS vorhanden und wurde getestet. USV-Systeme sind auf der Brücke und im Maschinenraum installiert.',
    rationale: 'Die Anforderung ist erfüllt. Das Notstromkonzept ist dokumentiert und wird regelmäßig getestet.',
    measure: '', criteria: [], effort: '', priority: '' },
  ...
  { id: 'UTN-1', article: 'E27-32 (SR 1.1 RE2)', name: 'Multifactor Authentication (Untrusted)', status: 'fail',
    gap: 'Keine Multi-Faktor-Authentifizierung für Remote-Zugriff über VSAT',
    evidence: 'Der Remote-Desktop-Zugang über VSAT erfolgt ohne MFA. Es wird ausschließlich eine Passwort-Authentifizierung verwendet.',
    rationale: 'Die Anforderung ist nicht erfüllt. IACS UR E27 Tabelle 2 verlangt eine Multi-Faktor-Authentifizierung bei Zugriffen über Untrusted Networks.',
    measure: '1. Einführung von MFA für alle Remote-Zugänge. 2. Einsatz von Hardware-Token oder App-basierter Authentifizierung.',
  ...
  { id: 'UTN-2', article: 'E27-36 (SR 1.13)', name: 'Access via Untrusted Networks', status: 'fail',
    gap: 'Keine Überwachung und Kontrolle des Shore-Zugangs; fehlende Netzwerksegmentierung',
    evidence: 'Crew-WiFi, administrative IT und OT-Systeme befinden sich im gleichen Netzwerksegment. Eine Firewall zwischen den Zonen existiert nicht. Der Shore-Zugang ist unkontrolliert.',
    rationale: 'Die Anforderung ist nicht erfüllt. Es liegt ein fundamentaler Verstoß gegen das Prinzip der Netzwerksegmentierung vor.',
    measure: '1. Einführung einer Netzwerksegmentierung (Crew/Admin/OT). 2. Installation einer Next-Generation-Firewall. 3. Einrichtung einer DMZ für die Shore-Kommunikation.',
    criteria: ['Dedizierte Netzwerksegmente für Crew, Admin und OT eingerichtet', 'Firewall an allen Zonenübergängen aktiv', 'Shore-Zugang ausschließlich über DMZ'],
    effort: '60-100h', priority: 'P0' },
  { id: 'UTN-3', article: 'E27-37 (SR 1.13 RE1)', name: 'Explicit Access Request Approval', status: 'fail',
    gap: 'Remote-Zugriff ohne On-Board-Freigabe durch Bordpersonal',
    evidence: 'Die Hersteller-VPN-Verbindung ist permanent aktiv. Eine Freigabe durch das Bordpersonal ist nicht erforderlich.',
    rationale: 'Die Anforderung ist nicht erfüllt. IACS UR E27 verlangt eine explizite On-Board-Genehmigung für jeden Remote-Zugriff.',
    measure: '1. VPN-Zugang ausschließlich nach expliziter Freigabe durch den Bordoffizier. 2. Durchgängiges Session-Logging. 3. Zeitliche Begrenzung der Zugänge.',
    criteria: ['Remote-Zugriff nur nach dokumentierter On-Board-Freigabe', 'Session-Logging aktiv', 'Zeitlich begrenzte Zugänge konfiguriert'],
    effort: '16-24h', priority: 'P0' },
];
