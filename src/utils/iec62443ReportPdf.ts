...
export async function generateIec62443Report(data: Iec62443ReportData): Promise<void> {
  ...
  const contextLead = lang === 'de'
    ? `Der vorliegende Bericht dokumentiert die Ergebnisse der Anwendbarkeitsprüfung des Schiffs bzw. Systems ${intakeData.facilityName} gemäß IACS UR E27. Die Bewertung wurde anhand der Anforderungen aus Table 1 und Table 2 der IACS UR E27 durchgeführt, wobei insgesamt ${threats.length} Bedrohungsszenarien analysiert und ${reqs.length} Anforderungen auf ihre Anwendbarkeit hin geprüft wurden.`
    : lang === 'fr'
    ? `Le présent rapport documente les résultats de l'évaluation d'applicabilité du navire/système ${intakeData.facilityName} conformément à l'IACS UR E27. L'évaluation a été réalisée sur la base des exigences des Tables 1 et 2 de l'IACS UR E27, portant sur ${threats.length} scénarios de menaces et l'applicabilité de ${reqs.length} exigences.`
    : `This report documents the results of the applicability review of vessel/system ${intakeData.facilityName} in accordance with IACS UR E27. The review assessed the requirements of Table 1 and Table 2 of IACS UR E27, covering ${threats.length} threat scenarios and the applicability of ${reqs.length} requirements.`;
  ...
  const verdictText = reviewSummary?.coreFinding
    ...
    : lang === 'de'
    ? noResidualScope
      ? `Im Rahmen der durchgeführten Anwendbarkeitsprüfung konnte festgestellt werden, dass für das Schiff bzw. System ${intakeData.facilityName} keine offenen Anforderungen im Restumfang (residual scope) der IACS UR E27 verbleiben.`
      : `Die Anwendbarkeitsprüfung des Schiffs bzw. Systems ${intakeData.facilityName} ergibt eine Abdeckungsrate von ${complianceRate} % gegenüber den Anforderungen der IACS UR E27. Insgesamt wurden ${critRisks.length} kritische Risiken sowie ${failReqs.length} vollständig anwendbare Anforderungen im Restumfang festgestellt, die vorrangige Behandlung erfordern.`
    : lang === 'fr'
    ? noResidualScope
      ? `L'évaluation d'applicabilité a permis de constater qu'aucune exigence de l'IACS UR E27 ne subsiste dans le périmètre résiduel du navire/système ${intakeData.facilityName}.`
      : `L'évaluation d'applicabilité du navire/système ${intakeData.facilityName} aboutit à un taux de couverture de ${complianceRate} % par rapport aux exigences de l'IACS UR E27. Au total, ${critRisks.length} risques critiques et ${failReqs.length} exigences pleinement applicables dans le périmètre résiduel ont été identifiés, nécessitant un traitement prioritaire.`
    : noResidualScope
    ? `The applicability review has determined that no IACS UR E27 requirements remain within the residual scope of vessel/system ${intakeData.facilityName}.`
    : `The applicability review of vessel/system ${intakeData.facilityName} yields a coverage rate of ${complianceRate}% against the requirements of IACS UR E27. A total of ${critRisks.length} critical risks and ${failReqs.length} fully applicable requirements in residual scope were identified, requiring priority treatment.`;
  ...
  pdf.complianceBar(passReqs.length, partialReqs.length, failReqs.length, {
    ...
    title: lang === 'de' ? 'IACS UR E27 Anwendbarkeitsverteilung' : lang === 'fr' ? 'Répartition de l\'applicabilité IACS UR E27' : 'IACS UR E27 Applicability Distribution',
  });
  ...
  const contextText = lang === 'de'
    ? `Der vorliegende Bericht dokumentiert die Ergebnisse einer strukturierten Anwendbarkeitsprüfung des Schiffs bzw. Systems ${intakeData.facilityName} gemäß den Anforderungen der IACS UR E27 (Cyber Resilience of On-Board Systems and Equipment). Die Prüfung wurde am ${dateStr} durchgeführt.\n\nZielsetzung der Bewertung war die systematische Identifikation und Bewertung von Bedrohungen für die an Bord installierten rechnergestützten Systeme (Computer Based Systems, CBS) sowie die Feststellung der Anwendbarkeit der 41 Anforderungen der IACS UR E27 (Tabelle 1 und Tabelle 2) und des daraus resultierenden Restumfangs.\n\nDer Bericht richtet sich an Reeder, Schiffsführung, den Electro-Technical Officer (ETO) bzw. IT-Verantwortlichen an Bord sowie an die zuständige Klassifikationsgesellschaft.`
    : `This report documents the results of a structured applicability review of vessel/system ${intakeData.facilityName} pursuant to the requirements of IACS UR E27 (Cyber Resilience of On-Board Systems and Equipment). The review was conducted on ${dateStr}.\n\nThe objective was the systematic identification and assessment of threats to Computer Based Systems (CBS) installed on board, as well as the determination of the applicability of the 41 requirements set out in IACS UR E27 (Table 1 and Table 2) and the resulting residual scope.\n\nThis report is intended for ship owners, vessel management, the Electro-Technical Officer (ETO) or IT responsible on board, and the relevant classification society.`;
