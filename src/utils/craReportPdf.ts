...
export function generateCraReport(data: CraReportData): void {
  ...
  const qgIntro = lang === 'de'
    ? 'Dieser Anhang dokumentiert die automatisierte Qualitaetssicherung gemäß vier Prüfdimensionen. Jeder Prüfpunkt wird maschinell gegen die Berichtsdaten validiert. Das Ergebnis (PASS / FAIL) ist revisionssicher und reproduzierbar.'
    : lang === 'fr'
  ...
  if (qaChecks && qaChecks.length > 0) {
    ...
    const qaIntroText = lang === 'de'
      ? `Die folgenden Prüfpunkte wurden durch die automatisierte Qualitaetssicherung identifiziert und — soweit möglich — automatisch korrigiert. ${qaIterLabel}`
      : lang === 'fr'
