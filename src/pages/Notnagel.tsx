...
function SectionHead({ step, title, lead }: { step: number; title: string; lead?: string }) {
  ...
  return (
    <div className="space-y-1.5">
      <p className="text-[10.5px] font-semibold uppercase tracking-[0.18em] text-[#00bcd4]">
        <Typewriter key={`e-${title}`} text={`Schritt ${step} von 5`} charDelay={14} cursor={false} onDone={advance(280)} />
      </p>
      <h2 className="text-2xl font-bold tracking-tight text-[#f5b800]">
...
const inputCls = "w-full rounded-none border border-[#2c3c52] bg-[#141c28] px-3 py-2.5 text-sm text-[#e8eef7] placeholder:text-[#5f6f86] shadow-none transition focus:outline-none focus:ring-2 focus:ring-[#f5b800]/35 focus:border-[#f5b800]";
          ...
          <section className="space-y-8">
            <div className="max-w-3xl space-y-4">
              <SeqText
                ...
                className="text-[10.5px] font-semibold uppercase tracking-[0.18em] text-[#00bcd4]"
                text="Business Continuity Management"
              />
              <SeqText
                order={1} current={seq} advance={advance} pause={640} charDelay={22} as="h2"
                className="text-3xl sm:text-[2.6rem] font-bold leading-[1.1] tracking-tight text-[#f5b800]"
