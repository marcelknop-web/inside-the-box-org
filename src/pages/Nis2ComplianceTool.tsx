...
function ReportView({ intakeData, risks, reqs }: { intakeData: Nis2IntakeData; risks: Nis2Risk[]; reqs: Nis2Req[] }) {
  const { t, language } = useLanguage();
  const [localRisks, setLocalRisks] = useState<Nis2Risk[]>(() => risks.map(r => ({ ...r, sources: [...r.sources] })));
  const [localReqs, setLocalReqs] = useState<Nis2Req[]>(() => reqs.map(r => ({ ...r, criteria: [...r.criteria] })));
  ...
  const handleQaCheck = useCallback(() => {
    ...
    setTimeout(() => {
      const result = runNis2QualityCheck(localRisks, localReqs, language as 'de' | 'en' | 'fr', intakeData);
  ...
  const handleApplyFixes = useCallback(() => {
    ...
    const t3 = setTimeout(() => {
      setFixProgress(75);
      const result = applyNis2AuditFixes(localRisks, localReqs, failed, language as 'de' | 'en' | 'fr', intakeData);
      ...
      setTimeout(() => {
        const newQa = runNis2QualityCheck(result.risks, result.reqs, language as 'de' | 'en' | 'fr', intakeData);
    ...
    requestAnimationFrame(() => {
      setTimeout(async () => {
        try {
          ...
          await generateNis2Report({ intakeData, risks: localRisks, reqs: localReqs, language: language as 'de' | 'en' | 'fr', entityTypeName: typeName, criticalityName: critName, isDraft: false, qaChecks: checksForPdf, fixLog: fixLogForPdf, qaIterations: qaIteration });
