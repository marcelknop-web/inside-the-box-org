  ...
  const contentMap: Record<string, () => ReactNode> = {
    ...
    'dora-nis2-ttx': () => (
      <TypedSection title={t('ttx.title')} mode="typewriter" intro={<p>{t('ttx.intro')}</p>}>
        ...
        <Block className="bg-highlight/10 border border-highlight/20 rounded-xl">
      ...
      <TypedSection title={t('imprint.title')} mode="typewriter" charDelay={8}>
        <Block className="bg-secondary/30">
          <p className="text-foreground text-sm md:text-[15px] font-sans leading-relaxed">
            ...
            {language === 'de' ? '34308 Bad Emstal, Deutschland' : language === 'fr' ? '34308 Bad Emstal, Allemagne' : '34308 Bad Emstal, Germany'}
          </p>
  ...
  const exampleQuestions = useMemo(() => {
    const all = {
      de: [
        ...
        'Assessment für unseren Sektor?',
        // Long — Regulatorik & Audit
        'Wie führt ihr eine NIS-2-Lückenanalyse mit klarer Roadmap zur Geschäftsleitung durch?',
        'Wie liefert ihr eine DORA-Gap-Analyse mit Fahrplan in sechs Wochen?',
        ...
        'Wie bereitet ihr uns operativ auf einen TIBER-EU-Test vor?',
        'Wie läuft ein begleitetes ISO 27001 Zertifizierungsprojekt mit euch ab?',
        'Wie kombiniert ihr BSI IT-Grundschutz und ISO 27001 in einem Steuerungssystem?',
        'Wie bereitet ihr uns auf ein TISAX-Assessment vor und welches AL ist realistisch?',
        'Wie ermittelt ihr unseren PCI-DSS-SAQ-Typ und das Merchant-Level?',
        'Wie strukturieren wir DORA Major-Incident-Klassifikation und NIS-2 24-h-Frühwarnung in einem Workflow?',
        'Wie integrieren wir DORA, NIS-2 und ISO 27001 zu einem konsistenten Steuerungssystem?',
        // Long — Krise, Training, vCISO
        ...
        'Wie würde ein Cyber Training Range Tag für unser SOC und IR-Team aussehen?',
        'Wie könnt ihr uns als virtuellen CISO für 6 bis 12 Monate begleiten?',
        'Könnt ihr ein Post-Incident-Review mit konkretem Verbesserungsplan durchführen?',
        'Wie dimensionieren wir einen IR-Retainer realistisch für unsere Größe?',
        // Long — strategische Audit-Findings
        'Wie weisen wir die Wirksamkeit unseres ISMS gegenüber dem Auditor nachvollziehbar nach?',
        'Wie definieren und operationalisieren wir einen tragfähigen Cyber-Risikoappetit?',
        ...
        'Wie heben wir TPRM von reaktiver Fragebogen-Akrobatik auf belastbares Lieferantenmanagement?',
      ...
      en: [
        ...
        'Assessment for our sector?',
        // Long — regulation & audit
        'How would you run a NIS-2 gap analysis with a clear board-level roadmap?',
        ...
        'How would you operationally prepare us for a TIBER-EU test?',
        'What does an accompanied ISO 27001 certification project with you look like?',
        'How do you combine BSI IT-Grundschutz and ISO 27001 into one steering system?',
        'How do you prepare us for a TISAX assessment and which AL level is realistic?',
        'How do you determine our PCI-DSS SAQ type and merchant level?',
        'How do we align DORA major-incident classification and NIS-2 24h early warning in one workflow?',
        'How do we integrate DORA, NIS-2 and ISO 27001 into one consistent steering system?',
        // Long — crisis, training, vCISO
        ...
        'What would a Cyber Training Range day for our SOC and IR team look like?',
        ...
        'How do we right-size an IR retainer for our scale?',
      ...
      fr: [
        // Short (≤34) — régulation & audit (services cœur)
        'NIS-2 art. 21 mesures min. ?',
        'DORA Art. 28 concentration ?',
        'DORA Art. 6 stratégie IT ?',
        'Cadrage TLPT TIBER-EU ?',
        'Préparer un test TIBER-EU ?',
        'PART-IS pour l\'aviation ?',
        'TISAX readiness en 4 sem. ?',
        'Choix du SAQ PCI-DSS ?',
        'Certification ISO 27001 ?',
        'BSI IT-Grundschutz maturité ?',
        'Conformité CRA sans CE ?',
        'Seuils Major Incident clairs ?',
        'BCM ISO 22301 maturité ?',
        'Directive CER vs NIS-2 ?',
        // Short (≤34) — crise, training, vCISO
        ...
        'Assessment pour notre secteur ?',
        // Long — régulation & audit
        'Comment menez-vous une gap analysis NIS-2 avec une roadmap claire pour le comex ?',
        ...
        'Comment nous préparer opérationnellement à un test TIBER-EU ?',
        'À quoi ressemble un projet accompagné de certification ISO 27001 avec vous ?',
        'Comment combinez-vous BSI IT-Grundschutz et ISO 27001 dans un système de pilotage ?',
        'Comment nous préparez-vous à un TISAX et quel AL est réaliste pour nous ?',
        'Comment déterminer notre type de SAQ PCI-DSS et notre niveau marchand ?',
        'Comment articuler la classification Major Incident DORA et l\'alerte 24h NIS-2 dans un seul workflow ?',
        'Comment intégrer DORA, NIS-2 et ISO 27001 dans un seul système de pilotage cohérent ?',
        // Long — crise, training, vCISO
        ...
        'À quoi ressemble une journée Cyber Training Range pour notre SOC et notre IR ?',
        ...
        'Comment dimensionner un retainer IR à notre échelle ?',
                  ...
                  <div className="relative flex items-center bg-secondary/90 backdrop-blur-md rounded-xl border border-highlight/30 focus-within:border-highlight/60 transition-electric shadow-lg focus-within:shadow-[0_0_24px_hsl(var(--highlight)/0.18)]">
                    ...
                    <textarea
                      ...
                      aria-label={activeService === 'crisis-sim' ? 'Chat input' : 'Ask the navigator'}
                       className="flex-1 min-w-0 bg-transparent px-2.5 md:px-3 py-2.5 text-base md:text-sm font-mono text-foreground placeholder:text-muted-foreground placeholder:transition-opacity placeholder:duration-500 resize-none focus:outline-none focus-visible:outline-none focus-visible:shadow-none max-h-[120px] truncate"
