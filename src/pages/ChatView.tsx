  ...
  const exampleQuestions = useMemo(() => {
    const all = {
      ...
      fr: [
        ...
        // Long — niveau expert
        'Comment cadrer un TLPT TIBER-EU de manière pertinente sur nos fonctions critiques ?',
        'Comment identifier et réduire les risques de concentration des tiers IT au sens de DORA Art. 28 ?',
        'Comment opérationnaliser la fonction Govern du NIST CSF 2.0 dans un SMSI groupe ?',
        'Comment mapper les SL-T IEC 62443-3-3 sur nos zones et conduits OT existants ?',
        'Comment mener un crown-jewel mapping clairement relié à la BIA et au threat modelling ?',
        'Comment bâtir un detection engineering crédible aligné MITRE ATT&CK avec une couverture mesurable ?',
        'Comment articuler nos tests de résilience avec les exigences de TLPT DORA ?',
        'Comment structurer la détection et réponse aux menaces identitaires sur AD, Entra ID et SaaS ?',
        'Comment transformer nos playbooks SOAR de scripts en actif d\'engineering maintenu ?',
        'Comment construire une stratégie de migration post-quantique pour PKI, TLS et code signing ?',
        'Comment rendre vraiment testable une stratégie de sortie cloud pour des workloads hyperscaler ?',
        'Comment intégrer les exigences SBOM du CRA et du SDLC sécurisé dans la gouvernance supply chain ?',
        'Comment agréger le risque technique en une position que le comex peut réellement piloter ?',
        'Comment opérationnaliser les trois lignes de défense entre IT, OT et sécurité groupe ?',
        'Comment articuler la classification Major Incident DORA, l\'alerte 24h NIS-2 et l\'art. 33 RGPD dans un seul workflow ?',
        'Comment intégrer les systèmes haut risque de l\'EU AI Act dans le SMSI et le registre des risques ?',
