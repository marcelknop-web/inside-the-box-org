  ...
  const exampleQuestions = useMemo(() => {
    const all = {
      ...
      en: [
        ...
        // Long — expert-level
        'How do we scope a TLPT under TIBER-EU meaningfully against our critical functions?',
        'How do we identify and reduce concentration risk for ICT third parties under DORA Art. 28?',
        'How do we operationalise the NIST CSF 2.0 Govern function inside a group-wide ISMS?',
        'How do we map IEC 62443-3-3 security-level targets onto existing OT zones and conduits?',
        'How do we run a crown-jewel mapping with a clean link to BIA and threat modelling?',
        'How do we build credible detection engineering against MITRE ATT&CK with measurable coverage?',
        'How do we align cyber resilience testing with DORA threat-led penetration testing requirements?',
        'How do we structure identity threat detection and response across AD, Entra ID and SaaS?',
        'How do we turn SOAR playbooks from script collections into a maintained engineering asset?',
        'How do we build a post-quantum migration strategy for PKI, TLS and code signing?',
        'How do we make cloud exit strategies for hyperscaler-bound workloads actually testable?',
        'How do we integrate SBOM requirements from EU CRA and secure SDLC into supply-chain governance?',
        'How do we aggregate technical risk into a position the board can actually steer with?',
        'How do we operationalise three lines of defence across IT, OT and group security?',
        'How do we align DORA major-incident classification, NIS-2 24h early warning and GDPR Art. 33 in one workflow?',
        'How do we bring EU AI Act high-risk systems into our ISMS and risk register?',
