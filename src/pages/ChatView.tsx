...
const ChatView = () => {
  ...
  const exampleQuestions = useMemo(() => {
    const all = {
      de: [
        // Short (≤34) — clevere Kundenfragen, mobil-tauglich
        'Wie messt ihr ISMS-Wirksamkeit?',
        'Was tun bei NIS-2 Verzug?',
        'Reicht ISO 27001 für NIS-2?',
        'DORA TLPT auch für uns?',
        'Wer haftet bei Auslagerung?',
        'Wie testen wir den IRP echt?',
        'SOC Eigenbau oder MSSP?',
        'EDR genug gegen Ransomware?',
        'Wie priorisieren wir CVEs?',
        'Was ist ein Major Incident?',
        'Wie übe ich den Krisenstab?',
        'Reicht MFA gegen Phishing?',
        'Was kostet ein Tabletop?',
        'Wie weit ist Zero Trust real?',
        'Backup vs. Immutable Storage?',
        'OT/IT-Trennung wie umsetzen?',
        'Was bringt ein Pentest wirklich?',
        'Welche KPIs für den Vorstand?',
        'Wie auditiere ich Lieferanten?',
        'Wann lohnt CISO as a Service?',
        'Wie melde ich nach 24h DSGVO?',
        'Was tun nach Ransomware?',
        'Cyberversicherung sinnvoll?',
        'Cloud Shared Responsibility?',
        'Wie sichere ich M365 wirklich?',
        'Wie reife ich von 2 auf 4?',
        'Reicht ein Awareness-Video?',
        'Wer prüft die Prüfer?',
        'KRITIS-Prüfung in 6 Monaten?',
        'Welche Belege will der Auditor?',
        // Short (≤34) — strategische Audit-Findings
        'ISMS-Wirksamkeit nicht belegt?',
        'Risikoappetit nicht definiert?',
        'Governance-Rollen unklar?',
        'Vorstand nie geübt?',
        'Krisenstab ohne Mandat?',
        'IKT-Strategie ohne Risiko?',
        'Auslagerungen nicht gesteuert?',
        'TPRM nur reaktiv?',
        'Schutzbedarf nie aktualisiert?',
        'Architektur ohne Zonenmodell?',
        'BIA fehlt für Kernprozesse?',
        'Resilienz nie ende-zu-ende geprüft?',
        'KPIs ohne Boardrelevanz?',
        'Mehrfach-Compliance nicht integriert?',
        'Lessons Learned versanden?',
        // Long — clevere Kundenfragen
        'Welche typischen Findings tauchen bei NIS-2 Audits zuerst auf?',
        'Wie überzeugen wir den Vorstand, dass Cybersecurity ein Investment ist?',
        'Was unterscheidet ein gutes von einem reinen Pflicht-Tabletop?',
        'Wie verhindern wir, dass unser ISMS reine Papierware bleibt?',
        'Wie messen wir den ROI unserer Security-Investitionen?',
        'Welche NIS-2 Pflichten werden in der Praxis am häufigsten unterschätzt?',
        'Wie weit reicht DORA wirklich in unsere IT-Lieferkette hinein?',
        'Wann lohnt sich ein virtueller CISO statt einer Festanstellung?',
        'Wie testen wir, ob unser Incident-Response-Plan unter Druck funktioniert?',
        'Welche Rolle spielt der Vorstand bei einem echten Cybervorfall?',
        'Wie sieht ein realistisches Ransomware-Szenario für uns aus?',
        'Was sind typische Schwachstellen in unserer Lieferkette?',
        'Wie balancieren wir Compliance und tatsächliche Sicherheit?',
        'Wie binden wir OT-Sicherheit sinnvoll in das ISMS ein?',
        'Welche Reifegrad-Stufe ist für uns realistisch erreichbar?',
        'Wie priorisieren wir Maßnahmen mit begrenztem Budget?',
        'Was passiert, wenn wir die NIS-2 Frist nicht halten?',
        'Wie sieht ein belastbarer Notfall-Kommunikationsplan aus?',
        'Wie prüfen wir die Cyberhygiene unserer Dienstleister?',
        'Wie gehen wir mit Schatten-IT und KI-Tools im Unternehmen um?',
        'Wie überleben wir einen Audit ohne Blackout-Momente?',
        'Was sollten wir vor einem Pentest unbedingt vorbereiten?',
        'Wie übersetzen wir Security-Risiken in Geschäftsrisiken?',
        'Welche Belege braucht ein NIS-2 Auditor wirklich?',
        'Wie etablieren wir eine ehrliche Fehlerkultur in der IT?',
        'Wie spielen wir realistische Cyberkrisen mit unserem Vorstand?',
        'Wie kombinieren wir DORA, NIS-2 und ISO 27001 ohne Doppelarbeit?',
        'Wie nehmen wir Fachbereiche bei der Risikoanalyse mit?',
        'Welche Rolle spielt KI in unserer Verteidigungsstrategie?',
        'Wie erkennen wir, ob ein MSSP wirklich liefert?',
        // Long — strategische Audit-Findings
        'Wie weisen wir die Wirksamkeit unseres ISMS gegenüber dem Auditor nachvollziehbar nach?',
        'Wie definieren und operationalisieren wir einen tragfähigen Risikoappetit?',
        'Wie heilen wir ein Finding zu unklaren Governance-Rollen zwischen CISO, CIO und Fachbereich?',
        'Wie überzeugen wir den Auditor, dass unser Vorstand Cyberkrisen wirklich führen kann?',
        'Wie statten wir den Krisenstab mit Mandat, Vollmachten und Entscheidungsfähigkeit aus?',
        'Wie verzahnen wir IKT-Strategie und Cyber-Risikomanagement gemäß DORA Art. 6?',
        'Wie schließen wir das Finding einer ungesteuerten oder konzentrierten IKT-Auslagerung?',
        'Wie heben wir TPRM von reaktiver Fragebogen-Akrobatik auf belastbares kritisches Lieferantenmanagement?',
        'Wie aktualisieren wir Schutzbedarf und Kronjuwelen ohne Top-down-Abkopplung?',
        'Wie führen wir ein belastbares Zonen- und Segmentierungsmodell für IT/OT/Cloud ein?',
        'Wie führen wir eine BIA so durch, dass sie BCM, DR und DORA-Resilienz wirklich trägt?',
        'Wie testen wir Cyber-Resilienz ende-zu-ende statt in isolierten technischen Übungen?',
        'Wie definieren wir Cyber-KPIs, die im Vorstand verstanden und genutzt werden?',
        'Wie integrieren wir DORA, NIS-2 und ISO 27001 zu einem konsistenten Steuerungssystem?',
        'Wie verankern wir Lessons Learned so, dass sie wiederkehrende Findings dauerhaft schließen?',
        // Short (≤34) — Projekt-Trigger
        'NIS-2-Quick-Assessment möglich?',
        'Habt ihr ein Erstgespräch frei?',
        'Tabletop für unseren Vorstand?',
        'vCISO für 6 Monate buchbar?',
        'Audit-Vorbereitung in 8 Wochen?',
        'DORA-Gap-Analyse machbar?',
        'TISAX-Readiness in 4 Wochen?',
        'Krisenübung im Q1 möglich?',
        'ISMS-Reifegradcheck in 10 Tagen?',
        'Lieferantenprogramm aufsetzen?',
        'Post-Incident-Review möglich?',
        // Long — Projekt-Trigger
        'Können wir mit einem zweistündigen Erstgespräch starten und dann entscheiden?',
        'Bietet ihr einen kompakten NIS-2-Quick-Check als Einstiegsprojekt an?',
        'Könnt ihr eine DORA-Gap-Analyse mit klarem Fahrplan in 6 Wochen liefern?',
        'Wie sieht ein 90-Tage-Programm zur Audit-Vorbereitung mit euch aus?',
        'Können wir euch als virtuellen CISO für 6 bis 12 Monate einsetzen?',
        'Wie würde ein begleiteter Tabletop für unseren Vorstand konkret ablaufen?',
        'Könnt ihr ein TISAX-Readiness-Assessment vor unserem Audit liefern?',
        'Wie würde ein ISMS-Reifegradcheck mit priorisierter Roadmap aussehen?',
        'Können wir mit einem Pilotprojekt starten, bevor wir uns langfristig binden?',
        'Übernehmt ihr die Begleitung unserer Audit-Phase als Sparringspartner?',
        'Könnt ihr ein Post-Incident-Review mit Verbesserungsplan durchführen?',
        'Helft ihr uns, ein belastbares Lieferantenprüfprogramm aufzusetzen?',
        'Können wir mit euch ein Cyber-Risikoreporting für den Vorstand entwickeln?',
        'Begleitet ihr uns bei der Einführung eines Krisenstab-Frameworks?',
        'Können wir ein Festpreis-Paket für unsere NIS-2-Umsetzung bekommen?',
      ],
      en: [
        // Short (≤34) — sharp customer questions
        'How do we prove ISMS works?',
        'NIS-2 deadline missed — now?',


        'Is ISO 27001 enough for NIS-2?',
        'Does DORA TLPT apply to us?',
        'Who is liable in outsourcing?',
        'How do we test our IRP?',
        'In-house SOC or MSSP?',
        'Is EDR enough vs ransomware?',
        'How do we prioritise CVEs?',
        'What is a Major Incident?',
        'How to drill the crisis team?',
        'Does MFA stop phishing?',
        'What does a tabletop cost?',
        'How real is Zero Trust today?',
        'Backups vs immutable storage?',
        'How to split OT and IT?',
        'What does a pentest show?',
        'Which KPIs for the board?',
        'How do we audit suppliers?',
        'When is a vCISO worth it?',
        'GDPR 72h — how to report?',
        'What to do after ransomware?',
        'Is cyber insurance worth it?',
        'Cloud shared responsibility?',
        'How to harden M365 properly?',
        'From maturity 2 to 4 — how?',
        'Is awareness video enough?',
        'Who audits the auditors?',
        'KRITIS audit in 6 months?',
        'What evidence do auditors want?',
        // Short (≤34) — strategic audit findings
        'ISMS effectiveness unproven?',
        'No defined risk appetite?',
        'Governance roles unclear?',
        'Board never exercised?',
        'Crisis team has no mandate?',
        'ICT strategy ignores risk?',
        'Outsourcing not steered?',
        'TPRM only reactive?',
        'Crown jewels never reviewed?',
        'No security zoning model?',
        'BIA missing for core processes?',
        'Resilience never end-to-end?',
        'KPIs without board relevance?',
        'Multi-regulation not aligned?',
        'Lessons learned go nowhere?',
        // Long — sharp customer questions
        'What are the most common findings in real NIS-2 audits?',
        'How do we convince the board that cybersecurity is an investment?',
        'What separates a great tabletop from a tick-the-box exercise?',
        'How do we keep our ISMS from becoming pure paperwork?',
        'How can we measure the ROI of our security investments?',
        'Which NIS-2 obligations are most often underestimated in practice?',
        'How far does DORA really reach into our IT supply chain?',
        'When does a virtual CISO make more sense than hiring one?',
        'How do we test if our incident response plan holds under pressure?',
        'What is the board\'s real role during a serious cyber incident?',
        'What does a realistic ransomware scenario look like for us?',
        'What are the typical weak spots in our supply chain?',
        'How do we balance compliance and actual security?',
        'How do we integrate OT security into our ISMS in a meaningful way?',
        'Which maturity level is realistic for us in the next 12 months?',
        'How do we prioritise measures with a limited budget?',
        'What happens if we miss the NIS-2 deadline?',
        'What does a robust crisis communication plan look like?',
        'How do we vet the cyber hygiene of our service providers?',
        'How do we deal with shadow IT and AI tools in the business?',
        'How do we survive an audit without blackout moments?',
        'What should we prepare before a pentest to get real value?',
        'How do we translate security risks into business risks?',
        'What evidence does a NIS-2 auditor really expect to see?',
        'How do we build an honest error culture inside IT?',
        'How do we run realistic cyber crises with our executive board?',
        'How do we combine DORA, NIS-2 and ISO 27001 without double work?',
        ...
        // Long — strategic audit findings
        'How do we credibly demonstrate ISMS effectiveness to the auditor?',
        'How do we define and operationalise a workable cyber risk appetite?',
        'How do we close findings on unclear governance roles between CISO, CIO and the business?',
        'How do we prove to the auditor that our board can actually lead a cyber crisis?',
        'How do we equip the crisis team with mandate, authority and decision power?',
        'How do we link ICT strategy and cyber risk management as required by DORA Art. 6?',
        'How do we close findings on uncontrolled or concentrated ICT outsourcing?',
        'How do we lift TPRM from reactive questionnaires to real critical-supplier steering?',
        'How do we update protection needs and crown jewels without losing top-down alignment?',
        'How do we introduce a credible zoning and segmentation model across IT, OT and cloud?',
        'How do we run a BIA that truly carries BCM, DR and DORA resilience?',
        'How do we test cyber resilience end-to-end instead of in isolated technical drills?',
        'How do we define cyber KPIs the board actually understands and uses?',
        'How do we integrate DORA, NIS-2 and ISO 27001 into one consistent steering system?',
        'How do we anchor lessons learned so recurring findings finally get closed?',
        // Short (≤34) — project triggers
        'NIS-2 quick assessment?',
        'Free slot for a first call?',
        'Tabletop for our board?',
        'vCISO for 6 months?',
        'Audit prep in 8 weeks?',
        'DORA gap analysis doable?',
        'TISAX readiness in 4 weeks?',
        'Crisis exercise in Q1?',
        'ISMS maturity check in 10d?',
        'Set up a TPRM programme?',
        'Post-incident review with us?',
        // Long — project triggers
        'Can we start with a 2-hour first call and decide from there?',
        'Do you offer a compact NIS-2 quick check as an entry project?',
        'Can you deliver a DORA gap analysis with a clear roadmap in 6 weeks?',
        'What would a 90-day audit-preparation programme with you look like?',
        'Can we engage you as a virtual CISO for 6 to 12 months?',
        'How would a board-level tabletop run by you concretely look?',
        'Can you deliver a TISAX readiness assessment before our audit?',
        'What would an ISMS maturity check with a prioritised roadmap look like?',
        'Can we start with a pilot before any longer-term commitment?',
        'Will you stay with us as a sparring partner during the audit phase?',
        'Can you run a post-incident review with a concrete improvement plan?',
        'Can you help us set up a robust supplier assurance programme?',
        'Can we develop a board-level cyber risk report together?',
        'Will you guide us through introducing a crisis-team framework?',
        'Can we get a fixed-price package for our NIS-2 implementation?',
      ],
      fr: [
        // Short (≤34) — questions clients pertinentes
        'Prouver l\'efficacité du SMSI ?',
        'NIS-2 en retard, on fait quoi ?',
        'ISO 27001 suffit pour NIS-2 ?',
        'TLPT DORA aussi pour nous ?',
        'Qui est responsable en outsourcing ?',
        'Comment tester notre IRP ?',
        'SOC interne ou MSSP ?',
        'EDR suffit vs ransomware ?',
        'Comment prioriser les CVE ?',
        'Qu\'est-ce qu\'un Major Incident ?',
        'Comment exercer la cellule ?',
        'MFA suffit-il vs phishing ?',
        'Combien coûte un tabletop ?',
        'Zero Trust, vraiment réaliste ?',
        'Backup ou stockage immuable ?',
        'Comment cloisonner OT et IT ?',
        'Que révèle vraiment un pentest ?',
        'Quels KPI pour le comité ?',
        'Comment auditer nos fournisseurs ?',
        'Quand choisir un vCISO ?',
        'Notification RGPD 72h ?',
        'Que faire après un ransomware ?',
        'Cyber-assurance utile ?',
        'Responsabilité partagée cloud ?',
        'Comment sécuriser M365 ?',
        'Passer de maturité 2 à 4 ?',
        'Vidéo de sensibilisation suffit ?',
        'Qui audite les auditeurs ?',
        'Audit KRITIS en 6 mois ?',
        'Quelles preuves veut l\'auditeur ?',
        // Short (≤34) — findings d'audit stratégiques
        'Efficacité du SMSI non prouvée ?',
        'Appétit au risque non défini ?',
        'Rôles de gouvernance flous ?',
        'Comex jamais exercé ?',
        'Cellule de crise sans mandat ?',
        'Stratégie IT sans risque ?',
        'Externalisation non pilotée ?',
        'TPRM uniquement réactif ?',
        'Joyaux jamais réévalués ?',
        'Pas de modèle de zonage ?',
        'BIA absente du critique ?',
        'Résilience jamais testée E2E ?',
        'KPI sans valeur pour le comex ?',
        'Multi-régul. non intégrée ?',
        'Lessons learned sans suite ?',
        // Long — questions clients pertinentes
        'Quels sont les findings les plus fréquents lors d\'un audit NIS-2 ?',
        'Comment convaincre le comex que la cybersécurité est un investissement ?',
        'Qu\'est-ce qui distingue un vrai tabletop d\'un simple exercice formel ?',
        'Comment éviter que notre SMSI ne devienne qu\'un dossier papier ?',
        'Comment mesurer le ROI de nos investissements de sécurité ?',
        'Quelles obligations NIS-2 sont les plus sous-estimées en pratique ?',
        'Jusqu\'où DORA s\'étend-il réellement dans notre chaîne IT ?',
        'Quand un CISO virtuel a-t-il plus de sens qu\'une embauche ?',
        'Comment tester si notre plan de réponse tient sous pression ?',
        'Quel est le vrai rôle du comex pendant un incident majeur ?',
        'À quoi ressemble un scénario ransomware réaliste pour nous ?',
        'Quels sont les points faibles typiques de notre supply chain ?',
        'Comment équilibrer conformité et sécurité réelle ?',
        'Comment intégrer la sécurité OT dans le SMSI de manière utile ?',
        'Quel niveau de maturité est réaliste pour nous à 12 mois ?',
        'Comment prioriser les mesures avec un budget limité ?',
        'Que se passe-t-il si nous manquons l\'échéance NIS-2 ?',
        'À quoi ressemble un plan de communication de crise solide ?',
        'Comment vérifier l\'hygiène cyber de nos prestataires ?',
        'Comment gérer le shadow IT et les outils IA en interne ?',
        'Comment passer un audit sans trous d\'air ?',
        'Que préparer avant un pentest pour qu\'il soit utile ?',
        'Comment traduire les risques cyber en risques business ?',
        'Quelles preuves un auditeur NIS-2 attend-il vraiment ?',
        'Comment instaurer une vraie culture de l\'erreur en IT ?',
        'Comment animer une vraie crise cyber avec notre comité ?',
        'Comment combiner DORA, NIS-2 et ISO 27001 sans doublons ?',
        ...
        // Long — findings d'audit stratégiques
        'Comment démontrer de manière crédible l\'efficacité de notre SMSI à l\'auditeur ?',
        'Comment définir et opérationnaliser un appétit au risque cyber tenable ?',
        'Comment lever un finding sur des rôles flous entre CISO, DSI et métiers ?',
        'Comment prouver à l\'auditeur que notre comex sait piloter une crise cyber ?',
        'Comment doter la cellule de crise d\'un mandat et d\'un vrai pouvoir de décision ?',
        'Comment articuler stratégie IT et gestion du risque cyber selon DORA Art. 6 ?',
        'Comment lever un finding sur une externalisation IT non pilotée ou concentrée ?',
        'Comment passer d\'un TPRM réactif à un vrai pilotage des prestataires critiques ?',
        'Comment mettre à jour besoins de protection et joyaux sans perdre l\'alignement top-down ?',
        'Comment introduire un modèle de zonage crédible IT/OT/cloud ?',
        'Comment mener une BIA qui porte vraiment PCA, PRA et résilience DORA ?',
        'Comment tester la résilience cyber de bout en bout, pas juste en silos techniques ?',
        'Comment définir des KPI cyber que le comex comprend et utilise vraiment ?',
        'Comment intégrer DORA, NIS-2 et ISO 27001 dans un seul système de pilotage cohérent ?',
        'Comment ancrer les lessons learned pour fermer les findings récurrents ?',
        // Short (≤34) — déclencheurs de projet
        'Quick assessment NIS-2 ?',
        'Créneau pour un premier RDV ?',
        'Tabletop pour notre comex ?',
        'vCISO sur 6 mois ?',
        'Préparation audit en 8 sem. ?',
        'Gap analysis DORA possible ?',
        'TISAX readiness en 4 sem. ?',
        'Exercice de crise au T1 ?',
        'Check maturité SMSI 10 j ?',
        'Programme TPRM à monter ?',
        'Post-incident review ?',
        // Long — déclencheurs de projet
        'Peut-on démarrer par un premier échange de 2h puis décider ensemble ?',
        'Proposez-vous un quick-check NIS-2 compact comme projet d\'entrée ?',
        'Pouvez-vous livrer une gap analysis DORA avec feuille de route en 6 semaines ?',
        'À quoi ressemblerait un programme de 90 jours de préparation à l\'audit ?',
        'Pouvons-nous vous engager comme CISO virtuel pour 6 à 12 mois ?',
        'Comment se déroulerait concrètement un tabletop animé pour notre comex ?',
        'Pouvez-vous livrer une évaluation de readiness TISAX avant notre audit ?',
        'À quoi ressemblerait un check de maturité SMSI avec une roadmap priorisée ?',
        'Pouvons-nous démarrer par un projet pilote avant tout engagement plus long ?',
        'Resterez-vous notre sparring-partner pendant toute la phase d\'audit ?',
        'Pouvez-vous mener un post-incident review avec un plan d\'amélioration ?',
        'Pouvez-vous nous aider à mettre en place un vrai programme TPRM ?',
        'Pouvons-nous co-construire un reporting cyber pour le comex ?',
        'Nous accompagnez-vous dans la mise en place d\'un cadre de cellule de crise ?',
        'Pouvons-nous obtenir un forfait au prix fixe pour notre mise en conformité NIS-2 ?',
      ],
    } as const;
