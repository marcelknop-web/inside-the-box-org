...
- Reporting truth: every entry in "reportingObligations" carries a "kind" that is exactly one of "Regulatory deadline", "Internal escalation target", "Company / contract / class target" or "Legal / operational escalation target". Statutory clocks are reproduced as written in law (NIS2 Art. 23: 24 h early warning, 72 h incident notification, 1 month final report; GDPR Art. 33: 72 h) — NEVER shorten a statutory clock to an hour-level target. Faster internal ambitions are separate entries with kind "Internal escalation target". IMO MSC-FAL.1/Circ.3 is cyber-risk-management guidance and sets no reporting deadline: CSO/SSO, flag state and class notifications are "Company / contract / class target", as are charterer, cargo-owner and terminal SLAs. Police / law enforcement / prosecutors are NEVER "Regulatory deadline" — there is no blanket statutory reporting clock for them; use "Legal / operational escalation target". Insurers, suppliers, vendors and service providers are contractual, never regulatory. State the clock as the regulatory window plus the resulting time, e.g. "24 h from 09:30 -> by 09:30 next day (T+24h)", and make sure every clock time written in a deadline is arithmetically identical to its T+ offset counted from classificationTime — a stated target time and its offset must never contradict each other.
...
13. Every time stated inside inject content matches that inject's own time; every clock time in a reporting deadline equals its own T+ offset counted from classificationTime.
14. No placeholder text anywhere ("xxx", "yyy", "TBD", "TODO", lorem ipsum) and no role task, prompt or responsibility repeated within the same role card.
15. Law enforcement, insurers, suppliers and vendors are never labelled "Regulatory deadline".
16. Any claim that credentials of a second system are exposed names the concrete mechanism (spoofed portal, DNS/proxy manipulation, credential reuse, token theft).
